...
const QMap<Settings::Environment, EnvironmentDescription>
    Settings::environmentDescriptions = {
    ...
    , m_tasksModel(new TaskManager::WindowTasksModel(this))
{
    ...
    if (!hasDesktopFileName()) {
        connect(
            m_tasksModel, &TaskManager::WindowTasksModel::rowsInserted, this,
        ...
        connect(
            m_tasksModel, &TaskManager::WindowTasksModel::dataChanged, this,
            [this](const QModelIndex& topLeft, const QModelIndex& bottomRight,
                ...
                if (!roles.isEmpty() && !roles.contains(TaskManager::AbstractTasksModel::LauncherUrlWithoutIcon) && !roles.contains(TaskManager::AbstractTasksModel::AppId)) {
                    return;
                }
...
bool Settings::setEnvironmentFromTasksModelIndex(const QModelIndex& idx)
{
    ...
    if (!service->categories().contains(QLatin1String("WebBrowser"))) {
        qInfo() << "Ignoring launcher URL" << launcherUrl
                << "which doesn't have \"WebBrowser\" category";
        return false;
    }
    ...
    m_tasksModel->disconnect(
        this); // prevent further signal emission to not deref a nullptr
...
void Settings::setEnvironmentFromExtensionMessage(const QJsonObject& data)
{
    ...
    // Most chromium-based browsers just impersonate Chromium nowadays to keep
    // websites from locking them out so we'll need to make an educated guess from
    // our parent process
    ...
    if (!hasDesktopFileName()) {
        ...
        qApp->setApplicationDisplayName(
            m_currentEnvironment.applicationDisplayName);
...
QJsonObject Settings::handleData(int serial, const QString& event,
    const QJsonObject& data)
{
    ...
    if (event == QLatin1String("getSubsystemStatus")) {
        // should we add a PluginManager::knownSubsystems() that returns a
        // QList<AbstractBrowserPlugin*>?
...
Settings::Environment Settings::environment() const { return m_environment; }
